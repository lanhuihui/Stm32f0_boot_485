...
/**
  ...
  * @param  startAddr: start of user flash area
...
uint32_t FLASH_If_Erase(uint32_t startAddr, uint32_t endAddr)
{
  ...
  /* Get the sector where start the user flash area */
	if(startAddr == APPHEADER_ADDRESS)
	{
		NbrOfErase = 1;
	}
	else
	{
#if FLASH_IS_32K
		NbrOfErase = (endAddr - startAddr + 1)/FLASH_PAGE_SIZE;
#elif FLASH_IS_64K
		NbrOfErase = (endAddr - startAddr + 1)/FLASH_PAGE_SIZE;
#elif FLASH_IS_1024K
		NbrOfErase = GetSector(endAddr) - UserStartSector + 1;
#endif
	}
  ...
  pEraseInit.NbSectors = NbrOfErase;
  pEraseInit.VoltageRange = VOLTAGE_RANGE_3;
#endif
	status = HAL_FLASHEx_Erase(&pEraseInit, &EraseError);
