...
PUTCHAR_PROTOTYPE
{
	...
	ITM_SendChar(ch);
#else
	HAL_UART_Transmit(&DEBUG_COM, (uint8_t *)&ch, 1, 0xFFFF);//DEBUG_COM USER_COM
#endif 
...
{
	uint16_t crc16 = oldCRC16;
	
#if 1
	while (len-- > 0)
		crc16 = CRC16_CCITT_Table[(crc16 >> 8 ^ *data++) & 0xff] ^ (crc16 << 8);
#else
	for(uint32_t i = 0; i < len; i++)
	{
		crc16 = CRC16_CCITT_Table[(crc16 >> 8 ^ *(data + i)) & 0xff] ^ (crc16 << 8);
	}
#endif	
	return crc16;
} 
...
void CRC16_Update(CRC16_CTX *ctx, const uint8_t *data, uint16_t len)
{
    for (uint16_t i = 0; i < len; i++) {
        ctx->crc = (ctx->crc << 8) ^ CRC16_CCITT_Table[(ctx->crc >> 8) ^ *data++];
    }
}
